...
class PaymentController extends Controller
{
    ...
    {
        $filters = $request->all('search', 'trashed', 'gateway', 'received');
        return Inertia::render('Payments/Index', [
            'filters'   => $filters,
            'payments'  => new PaymentCollection(
                Payment::filter($filters)->latest()->paginate()->withQueryString()
            ),
        ]);
    }
    ...
    public function status(Request $request, Payment $payment)
    {
        ...
        if ($user && ($user->owner || $user->id == $payment->user_id)) {
            ...
            return back()->with('success', __('model_marked', ['model' => __choice('Payment', 1), 'status' => __('unconfirmed')]));
        }
