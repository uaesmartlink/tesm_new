<template>
  <div v-if="order" class="w-full -my-6">
    <div class="w-full bg-white rounded shadow-lg leading-normal p-4">
      ...
      <div class="-m-4 px-8 py-4 bg-gray-100 border-t border-gray-200 flex items-center rounded-b">
        ...
        <loading-button type="button" @click="sendOrder" :loading="sending" class="btn-gray px-4 py-2 ml-auto flex items-center">
  ...
  <div v-if="measurement" class="w-full -my-6">
    <div class="w-full bg-white rounded shadow-lg leading-normal p-4">
      <h4 class="font-bold">{{ $t('Send SMS to Customer') }}</h4>
      <h4 class="text-sm mt-2 mb-4" v-html="$t('sending_to_html', { name: this.measurement.customer.name, phone: this.measurement.customer.phone })"></h4>
      <div v-if="sent" class="text-green-500 mb-4">
        {{ $t('SMS has been successfully sent to customer.') }}
      </div>
      <div v-else>
      ...
      <div class="-m-4 px-8 py-4 bg-gray-100 border-t border-gray-200 flex items-center rounded-b">
        ...
        <loading-button type="button" @click="sendMeasurement" :loading="sending" class="btn-gray px-4 py-2 ml-auto flex items-center">
          <div v-if="sending" class="btn-spinner ltr:mr-2 rtl:ml-2" />
          {{ $t('Send') }}
        </loading-button>
      </div>
...
export default {
  ...
  props: { order: Object, measurement : Object},
  data() {
  ...
  methods: {
    ...
    sendMeasurement() {
      ...
      console.log(this.measurement.appointment);
      ...
      this.sending = true;
      this.$axios
        .post(this.route('customers.sms.measurement', this.measurement.id), { text: this.text })
        .then(res => {
          this.sending = false;
          this.sent = res.data.success;
          if (res.data.success) {
