<template>
  <nav class="md:hidden bottom-0 w-full h-12 bg-gray-800 text-xs  mt-20">
      ...
      <template v-for="(m, i) in menus" :key="'mm_' + i">
        <li class="relative p-3" :class="isUrl(m.link) ? 'w-12 mt-2' : ''">
          <Link
            ...
            "
          >
            <Icons :name="m.icon" class="mx-auto" :class="isUrl(m.link) ? 'h-8 w-8 fill-white ' : 'h-6 w-6 fill-current'" />
          </Link>
        </li>
      </template>
    </ul>
