<template>
  <div>
      ...
      <div class="h-screen flex flex-col" @click="hideDropdownMenus">
        <div class="md:flex np" style="min-height: 56px">
          <div class="titlebar bg-gray-900 md:flex-shrink-0 md:w-64 px-4 py-3 flex items-center justify-center md:justify-start">
            <Link href="/">
              <div class="h-8 text-white text-lg py-1 flex items-center">
                <Icons name="logo" class="w-6 h-6 ltr:mr-2 rtl:ml-2 fill-gray-300 group-hover:fill-white" />
            ...
            <div class="flex items-center md:hidden ltr:ml-auto rtl:mr-auto">
              <!-- <dropdown placement="bottom-end" class="ltr:mr-4 rtl:ml-4">
                <flag :iso="$languages[language || 'en'].flag" :squared="false" class="m-1 sm:m-2" style="width: 1.1em !important" />
                <template v-slot:dropdown>
                  <div class="mt-3 py-4 pl-0 pr-0 md:pr-4 shadow-lg bg-gray-800 rounded text-sm">
                    <button
                      ...
                      class="block w-full text-left px-6 py-3 text-white hover:bg-gray-900 hover:text-white"
                      :class="{ 'bg-gray-900': language == lang }"
                    >
                      <flag :iso="$languages[lang].flag" :squared="false" class="ltr:mr-2 rtl:ml-2" />
              ...
              <dropdown placement="bottom-end" class="ltr:mr-4 rtl:ml-4">
                <Icons name="bars" class="fill-white w-4 h-4 m-1 sm:m-2" />
                <template v-slot:dropdown>
                  <div class="mt-3 py-4 pl-0 pr-0 md:pr-4 shadow-lg bg-gray-800 rounded text-sm">
              ...
              <dropdown placement="bottom-end" class="ltr:mr-4 rtl:ml-4">
                <Icons name="add" class="fill-white w-4 h-4 m-1 sm:m-2" />
                <template v-slot:dropdown>
              ...
              <dropdown placement="bottom-end" class="ltr:mr-2 rtl:ml-2">
                <Icons name="users" class="fill-white w-4 h-4 m-1 sm:m-2" />
                <template v-slot:dropdown>
                  <div class="mt-3 py-2 shadow-lg bg-gray-800 rounded text-sm">
                    <user-menu :url="url()" />
                  </div>
    ...
    <mobile-menu :url="url()" class="np z-20 fixed bottom-0 w-full px-2 mt-72"></mobile-menu>
    <v-dialog />
