  ...
  <div id="nav" class="relative px-4">
    ...
    <template v-for="(m, i) in menus" :key="'mm_' + i">
      <div
        ...
        :class="selected === i ? 'z-10 text-gray-100 ltr:ml-2 rtl:mr-2' : 'ltr:ml-2 rtl:mr-2 text-gray-300 hover:text-white'"
      >
        <Link class="flex items-center group py-3" :href="route(m.link)">
          <Icons
            :name="m.icon"
            class="w-4 h-4 ltr:mr-4 rtl:ml-4"
            :class="selected === i ? 'z-10 fill-gray-100' : 'fill-gray-300 group-hover:fill-white'"
          />
    ...
    <template v-if="$user.owner">
      <template v-for="(m, mi) in owner_menus" :key="'omm_' + (mi + mil)">
        <div
          ...
          :class="selected === mi + mil ? 'z-10 text-gray-100 ml-2' : 'ml-2 text-gray-300 hover:text-white'"
        >
          <Link class="flex items-center group py-3" :href="route(m.link)">
            <Icons
              :name="m.icon"
              class="w-4 h-4 ltr:mr-4 rtl:ml-4"
              :class="selected === mi + mil ? 'z-10 fill-gray-100' : 'fill-gray-300 group-hover:fill-white'"
            />
            <div class="z-10">{{ $tc(m.title, 2) }}</div>
          </Link>
        </div>
