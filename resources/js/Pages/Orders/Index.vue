    ...
    <div>
      ...
      <div class="np mb-6 flex justify-between items-center">
        <search-filter
          ...
          @reset="reset"
        >
          <label class="block text-gray-700">{{ $tc("Delivery", 2) }}</label>
          <v-select
            v-model="form.due_in"
            :options="[
              { value: '', label: $t('All') },
      ...
      <div
        class="bg-white rounded shadow overflow-x-auto scroll-on-light overflow-y-hidden"
      >
...
export default {
  ...
  components: { Show, Modal, Pagination, SearchFilter, PaymentForm },
  // metaInfo() {
    ...
    form: {
      handler: throttle(function () {
        ...
        this.$inertia.replace(
          this.route("orders", Object.keys(query).length ? query : { remember: "forget" })
        );
      }, 250),
      deep: true,
    },
  ...
  methods: {
    ...
    PriorityCol(p) {
      if (p == 1) {
        return (
          '<span class="bg-gray-200 rounded text-sm py-1 px-2">' +
          this.$t("Normal") +
          "</span>"
        );
      } else if (p == 2) {
        return (
          '<span class="bg-orange-200 rounded text-sm py-1 px-2">' +
          this.$t("High") +
          "</span>"
        );
      } else if (p == 3) {
        return (
          '<span class="bg-red-200 rounded text-sm py-1 px-2">' +
          this.$t("Urgent") +
          "</span>"
        );
      }
