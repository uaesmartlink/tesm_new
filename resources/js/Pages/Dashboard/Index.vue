<template>
  <div>
    <div class="flex overflow-x-auto scroll-on-light pb-2">
      <div
        class="w-full max-w-xs md:max-w-md lg:w-1/4 min-w-225 flex text-white items-center bg-green-to-teal rounded shadow p-4 ltr:mr-6 rtl:ml-6"
      >
        <span
          class="flex items-center justify-center h-12 w-12 rounded-full shadow bg-mono-circle ltr:mr-4 rtl:ml-4"
        >
        ...
        <div>
          <h1 class="text-2xl">
            ...
            <span class="text-sm">
              {{ $shortNumber(data.order.total_count) }}
              {{ $tc("Order", data.order.total_count) }}</span
            >
          </h1>
      ...
      <div
        class="w-full max-w-xs md:max-w-md lg:w-1/4 min-w-225 flex text-white items-center bg-blue-to-purple rounded shadow p-4 ltr:mr-6 rtl:ml-6"
      >
        <span
          class="flex items-center justify-center h-12 w-12 rounded-full shadow bg-mono-circle ltr:mr-4 rtl:ml-4"
        >
        ...
        <div>
          <h1 class="text-2xl">
            ...
            <span class="text-sm">
              {{ $shortNumber(data.salary.total_count) }}
              {{ $tc("Salary", data.salary.total_count) }}</span
            >
          </h1>
          <p class="text-xs mt-2">{{ data.start }} - {{ data.end }}</p>
        </div>
      ...
      <div
        class="w-full max-w-xs md:max-w-md lg:w-1/4 min-w-225 flex text-white items-center bg-red-to-orange rounded shadow p-4 ltr:mr-6 rtl:ml-6"
      >
        <span
          class="flex items-center justify-center h-12 w-12 rounded-full shadow bg-mono-circle ltr:mr-4 rtl:ml-4"
        >
        ...
        <div>
          <h1 class="text-2xl">
            ...
            <span class="text-sm">
              {{ $shortNumber(data.expense.total_count) }}
              {{ $tc("Expense", data.expense.total_count) }}</span
            >
          </h1>
          <p class="text-xs mt-2">{{ data.start }} - {{ data.end }}</p>
        </div>
      ...
      <div
        class="w-full max-w-xs md:max-w-md lg:w-1/4 min-w-225 flex text-white items-center bg-blue-to-teal rounded shadow p-4"
      >
        <span
          class="flex items-center justify-center h-12 w-12 rounded-full shadow bg-mono-circle ltr:mr-4 rtl:ml-4"
        >
    ...
    <div class="w-full flex flex-wrap items-center mt-4">
      <div id="month" class="w-full lg:w-1/2">
        <div
          class="bg-month-chart rounded shadow py-4 px-6 mb-6 lg:mb-0 ltr:mr-0 lg:ltr:mr-3 rtl:ml-0 lg:rtl:ml-3"
        >
          <div
            class="font-extrabold mb-6 -my-4 -mx-6 py-4 px-6 border-b border-gray-900 flex items-center"
          >
            <div class="flex-1 text-lg text-gray-300">
              {{ $t("Month Overview Chart") }}
            </div>
      ...
      <div id="year" class="w-full lg:w-1/2">
        <div
          class="bg-year-chart rounded shadow py-4 px-6 ltr:ml-0 lg:ltr:ml-3 rtl:mr-0 lg:rtl:mr-3"
        >
          <div
            class="font-extrabold mb-6 -my-4 -mx-6 py-4 px-6 border-b border-blue-900 flex items-center"
          >
            <div class="flex-1 text-lg text-gray-300">
              {{ $t("Year Overview Chart") }}
            </div>
            <div class="inline-block relative text-gray-400">
              <select
                v-model="year"
    ...
    <div class="w-full flex flex-wrap items-stretch my-6">
      <div id="no" class="w-full lg:w-1/2">
        <div
          class="bg-green-to-teal rounded shadow py-4 px-6 mb-6 lg:mb-0 ltr:mr-0 lg:ltr:mr-3 rtl:ml-0 lg:rtl:ml-3"
        >
          <div
            class="font-extrabold text-white mb-6 -my-4 -mx-6 py-4 px-6 border-b border-o2 text-lg"
          >
            {{ $t("Today Orders") }}
          </div>
          <div class="pb-4 overflow-auto scroll-on-dark">
            ...
            <div v-else id="new" class="min-w-600" style="max-height: 350px">
              <table class="w-full text-white whitespace-no-wrap">
                <thead>
                ...
                <tbody>
                  ...
                  <tr :key="order.id" v-for="order in new_today">
                    ...
                    <td class="border-t border-o2 px-3 py-2 text-center">
                      ...
                      <div
                        v-if="order.priority"
                        v-html="PriorityCol(order.priority)"
                      ></div>
                    </td>
                    <td class="border-t border-o2 text-right font-bold px-3 py-2">
                      <div class="inline-flex">
                        <Icons
                          v-if="order.paid"
                          name="check"
                          class="flex-shrink-0 w-3 h-3 fill-white ltr:mr-2 rtl:ml-2"
                        />
                        {{ order.grand_total }}
                      </div>
                    </td>
      ...
      <div id="do" class="w-full lg:w-1/2">
        <div
          class="bg-red-to-orange rounded shadow py-4 px-6 mb-6 lg:mb-0 ltr:ml-0 lg:ltr:ml-3 rtl:mr-0 lg:rtl:mr-3"
        >
          <div
            class="font-extrabold text-white mb-6 -my-4 -mx-6 py-4 px-6 border-b border-o2 text-lg"
          >
          ...
          <div class="pb-4 overflow-x-auto scroll-on-dark">
            <div v-if="due_today.length === 0" class="text-white">
    ...
    setYearChart(chart) {
      ...
      this.yearChartOptions.tooltip.pointFormatter = function (tooltip) {
        ...
        return `<tr><td style="color:${
          this.series.color
        };padding:0;font-weight:bold;">${tc(
          vm.$capitalize(this.series.name)
        )}: &nbsp;&nbsp;</td><td style="color:${
          this.series.color
        };padding:0;text-align:right"><b>${this.y}</b></td></tr>`;
      };
    },
    setMonthChart(chart) {
      let series = [];
      ...
      Object.keys(chart.order)
        ...
        .map((k) => {
          let day = new Date(k).toLocaleString("en-us", {
            day: "numeric",
            weekday: "short",
          });
          categories.push(day);
