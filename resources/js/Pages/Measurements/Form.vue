<template>
  <div class="w-full ltr:text-left rtl:text-right">
    <div
      :class="
        modal
          ? 'np flex flex-col justify-between items-center px-6 py-4 mb-4 bg-gray-100 border-b rounded-t'
          : ''
      "
    >
      ...
      <div :class="modal ? 'print bg-white -mx-6 -mb-8 rounded-md' : ''">
        <form @submit.prevent="submit" autocomplete="off">
          <div
            class="px-8 py-8 ltr:-mr-6 rtl:-ml-6 ltr:ml-0 rtl:mr-0 -mb-8 flex flex-wrap"
          >
              ...
              <v-select
                ...
                :class="{
                  error:
                    ...
                    $page.props.errors?.service_id.length,
                }"
              ></v-select>
            ...
            <div class="mb-8 ltr:mr-6 rtl:ml-6 flex flex-wrap">
              ...
              <div>
                <div class="text-sm mt-1">
                  {{
                    $t(
                      "Person name with services name i.e, John's Son Blazer, Mike Kurta or Shalini Blouse etc"
                    )
                  }}
                </div>
                <div
                  v-if="$page.props.errors?.name && $page.props.errors?.name.length"
                  class="form-error"
                >
            ...
            <div
              ...
              v-if="
                service && service.measurement_fields && service.measurement_fields.length
              "
            >
              <h1 class="font-bold mb-2">{{ $tc("Measurement Field", 2) }}</h1>
              <hr class="mb-8" />
              <div
                class="ltr:-mr-6 rtl:-ml-6 ltr:ml-0 rtl:mr-0 col-span-full grid grid-cols-1 sm:grid-cols-2"
              >
                <template v-for="(f, i) in service.measurement_fields">
                  ...
                  <template v-else-if="f.type == 'Checkbox'">
                    <div :key="'in_' + i">
                      <div
                        class="ltr:text-left rtl:text-right ltr:pr-6 rtl:pl-6 mb-8"
                        :key="'in_' + i"
                      >
                        <label class="form-label mr-6" :key="'in_' + i">{{
                          f.name
                        }}</label>
                        <div class="py-2"></div>
                        <checkbox-input
                        ...
                        <div
                          v-if="
                            $page.props.errors[f.name] &&
                            $page.props.errors[f.name].length
                          "
                          class="form-error"
                        >
...
export default {
  ...
  watch: {
    ...
    customer: function (cv, ov) {
      ...
      if (
        ...
        this.form.name ==
          (ov ? ov.label + "'s " : "") + (this.service ? this.service.label : "")
      ) {
        this.form.name =
          (cv ? cv.label : "") + (this.service ? "'s " + this.service.label : "");
      }
    },
    service: function (cv, ov) {
      ...
      if (
        ...
        this.form.name ==
          (this.customer ? this.customer.label + "'s " : "") + (ov ? ov.label : "")
      ) {
        this.form.name =
          (this.customer ? this.customer.label + "'s " : "") + (cv ? cv.label : "");
      }
      if (ov) {
  ...
  methods: {
    submit() {
      ...
      if (
        this.service &&
        this.service.measurement_fields &&
        this.service.measurement_fields.length > 0
      ) {
        let extra = [];
        let extras = this.service.measurement_fields.map((f, i) => {
          ...
          let field = {};
          field[f.name] =
            f.type == "Checkbox" ? JSON.stringify(this.form[f.name]) : this.form[f.name];
          extra[i] = { ...field };
        });
    ...
    searchingServices: debounce((loading, search, vm) => {
      fetch(vm.route("ajax.services") + "?with_fields=1&search=" + escape(search)).then(
        (res) => {
