          ...
          >
            <td class="border-t px-6 py-4 w-56">
              <div class="flex items-center">
                {{ measurement.name }}
                <Icons
                  v-if="measurement.deleted_at"
                  name="trash"
                  class="flex-shrink-0 w-3 h-3 fill-gray-400 ltr:ml-2 rtl:mr-2"
                />
              </div>
            </td>
...
export default {
  ...
  methods: {
    ...
    getCustomerName(measurement) {
      fetch(
        this.route("measurements.show", [measurement.id, md5(measurement.name)]) +
          "?ajax=1"
      ).then((res) => {
        res.json().then((data) => {
