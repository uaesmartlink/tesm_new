      ...
      <div class="print">
        <div class="max-w-2xl h-full py-4 px-6 leading-normal">
            ...
            <div class="text-sm">
              ...
              <div class="flex">
                ...
                <span class="font-bold">
                  <span class="inline-block">{{ $date(payment.created_at) }}</span>
                  <span class="inline-block">{{ $time(payment.created_at) }}</span>
                </span>
              </div>
              <div class="flex">
                <span class="text-right ltr:mr-2 rtl:ml-2" style="width: 90px">
          ...
          <a
            ...
            :href="`/img/${payment.photo_path}?ref=${
              $page.props.user ? $page.props.user.id + '_' + payment.hash : payment.hash
            }`"
          >
            {{ $t("View Payment Photo") }}
          </a>

          <div id="details" class="flex flex-wrap xs:flex-no-wrap my-6">
            <div id="company" class="pr-0 xs:pr-3 w-full xs:max-w-1/2">
          ...
          <div class="mt-6 px-4 py-3 border rounded font-bold text-lg">
            <h1 class="border-b pb-4 px-4 -mx-4">
              {{ payment.received ? $t("Payment Note") : $t("Payment Request") }}
            </h1>
            ...
            <div v-if="payment.received">
              ...
              <div
                ...
                class="text-sm flex items-center justify-between mt-1"
              >
            ...
            <div
              v-else
              class="flex items-center mt-4 px-4 py-3 rounded bg-red-500 text-white text-sm"
            >
  ...
  methods: {
    ...
    receivedPayment() {
      if (!this.payment.received) {
        this.$emit("close");
        // TODO Confirm first
        this.$inertia.put(this.route("payments.status", this.payment.id), {
          received: 1,
        });
      }
    },
